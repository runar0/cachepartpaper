...
\runar{Figure out how to present latency values in a table without filling half a page ...}
\runar{Depending on which results we'll include, we might only have one second-level cache configuration}
\runar{Section about the limited memory model in sniper? or in a later section?}
...
\runar{Specifying all workloads in each group will take multiple pages, but it might not be ok to leave out?}
...
\runar{MPKI might be well enough know to skip the definition?}